...
def sine(trajectories_to_sample, device):
    ...
    t = torch.linspace(t_begin, t_end, t_nsamples).to(device)
    y = torch.sin(t)
...
def parse_datasets(args, device):
    ...
    if dataset_name == "sine":
        ...
        train_dataloader = DataLoader(train_trajectories, batch_size=batch_size, shuffle=False,
                                      collate_fn=lambda batch: basic_collate_fn(batch, t, data_type="train"))
        test_dataloader = DataLoader(test_trajectories, batch_size=batch_size, shuffle=False,
    ...
    if dataset_name == "hopper":
        ...
        time_steps = torch.arange(
            start=0, end=n_tp_data, step=1).float().to(device)
